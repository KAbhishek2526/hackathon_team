...
export default function LoginPage() {
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        setLoading(true);
        try {
            const res = await api.login(form) as { token: string; user: { role?: string } };
            saveToken(res.token, res.user);
            // Redirect based on role
            const role = res.user?.role;
            window.location.href = role === 'global_client' ? '/pro-dashboard' : '/dashboard';
        } catch (err: unknown) {
    ...
    return (
        <div className="min-h-[80vh] flex items-center justify-center">
            <div className="w-full max-w-md bg-slate-900 border border-slate-800 rounded-xl p-8">
                ...
                <p className="text-slate-400 text-sm mb-6">Welcome back — students and professionals alike.</p>
